...
export const Cell = ({onPress, number, figure, winner, winningPattern}) => {
  ...
  const figures = {
    ...
    CROSS: (
      <Cross
        stroke={
          winner ? (winner === figure ? Colors.WHITE : Colors.YELLOW) : null
        }
      />
    ),
    CIRCLE: (
  ...
  return (
    <Pressable style={styles.container} onPress={() => onPress(number)}>
      <View style={styles.stepFigure}>
        <Rectangle
          fill={winningPattern?.includes(number) ? Colors.BLUE : null}
        />
      </View>
      <View style={styles.stepFigure}>{figures[figure]}</View>
    </Pressable>
  );
};
